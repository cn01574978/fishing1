	...
	public Sprite createNextButton(final FishJoy activity, final Context context, TextureRegion nextTextureRegion) {
		...
		Sprite nextButton = new Sprite(CAMERA_WIDTH * 0.5f + 100, CAMERA_HEIGHT * 0.5f, nextTextureRegion) {
			...
			public boolean onAreaTouched(final TouchEvent pSceneTouchEvent, final float pTouchAreaLocalX, final float pTouchAreaLocalY) {
				...
				dialog.setView(view);
				dialog.setPositiveButton("确锟斤拷", new DialogInterface.OnClickListener() {
				      public void onClick(DialogInterface dialog, int which) {
				          //unimplemented method 
				    	  EditText editText=(EditText)view.findViewById(R.id.edittext);
				    	  if(editText.getText().toString().equals(""))
				    	  {
				    		  Toast.makeText(activity, "锟斤拷锟斤拷锟斤拷锟斤拷锟脚", Toast.LENGTH_SHORT).show();
				    		  dialog.cancel();
				    	  }
				    	  else
				    	  {
				...
				dialog.setNegativeButton("取锟斤拷", new DialogInterface.OnClickListener() {
				      public void onClick(DialogInterface dialog, int which) {
				    	 //unimplemented method 
				    	  dialog.dismiss();
				         }});
		...
		Text mScoreText = this.createText(mScoreFont,"SCORE: " + mScore,+10);
			...
			if (mScore >= Max)
			{
				if(!isEstablished){
					...
					Text inputText = new Text(0, 0, mTimeFont, "锟斤拷锟斤拷锟斤拷锟铰的硷拷录锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷拇锟斤拷锟", HorizontalAlign.CENTER);
